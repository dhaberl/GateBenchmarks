...
def relative_uncertainty(folders, scale, factor):
    ...
    print('Data: min max mean med', mi, ma, mean, med) 
    q = np.amax(data2)
    print('Data max = ', q)
    ...
    print('Length data  =', len(d1), len(d2))
    print('Length sigma =', len(sigma1))
    u1 = uncert1[mask == 1]
    u2 = uncert2[mask == 1]
    ...
    l1 = '{:15s} $\mu=${:.2f}% '.format('PHSP1 vs GAN', np.mean(diff1)*100.0)#, np.mean(sigma1)/np.mean(d1)*100.0)
    print(l1)
    ...
    if  np.abs(np.mean(x)) < 0.1 and  np.std(x) < 0.5:
        return True
    return False
...
def analyse_all_folders(output_folders, **kwargs):
    ...
    for o in outputFolders:
        r = relative_uncertainty(['reference_data', o], [1, 10], 0)
